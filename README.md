...
Лабораторная работа посвящена реализации персистентного словаря на основе левостороннего красно-чёрного дерева. Структура соответствует требованиям: неизменяемость, поддержка полиморфных ключей/значений, операции высшего порядка и проверка свойств через property-based тестирование.

### Особенности реализации
- **Структура данных**: модуль `Fplab1.RBDict`, использующий кортежи `{:node, color, key, value, left, right}` для описания левостороннего красно-чёрного дерева. Балансировка достигается через явные повороты и перекраску узлов.
- **Функции высшего порядка**: `map/2`, `filter/2`, `foldl/3`, `foldr/3` написаны в том же стиле — это простые рекурсивные обходы дерева с сопоставлением кортежей.
